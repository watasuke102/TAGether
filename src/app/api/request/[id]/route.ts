...
export async function PUT(req: Request, { params }: { params: Promise<{ id: string }> }): Promise<Response> {
  ...
  const result = await db.update(request).set({ answer }).where(eq(request.id, Number((await params).id)));
  const updated_req = await db.select().from(request).where(eq(request.id, Number((await params).id)));
  con.end();
